...
{
    public function uploadAndParse(UploadedFile $file, User $user): array
    {
        // S3에 이미지 업로드
        $path = Storage::disk('s3')->put(
            'running-logs/' . $user->id . '/' . now()->format('Ymd'),
            $file,
            'public'
        );
        ...
        try {
            ...
            Log::warning('CORE API 이미지 파싱 실패: ' . $e->getMessage());
        }

        return ['image_url' => $imageUrl, 'parsed_data' => $parsedData];
    }
