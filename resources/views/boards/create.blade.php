...
<div class="max-w-3xl mx-auto px-4 py-8 md:px-6 lg:px-8">
  ...
  <div class="bg-pac-black-900 border border-pac-black-100">
    ...
    {{-- 카드 헤더 --}}
    <div class="px-6 py-4 border-b border-pac-black-100 flex items-center justify-between">
      <h1 class="font-display text-xl uppercase tracking-wider text-white">
        새 게시글 <span class="text-pac-yellow-500">— {{ $meta['label'] }}</span>
      </h1>
    </div>

    <form method="POST" action="{{ route('boards.store', $type) }}" class="p-6 space-y-5">
      @csrf
      ...
      <div class="flex items-center justify-between pt-2 border-t border-pac-black-100">
        <a href="{{ route('boards.index', $type) }}" class="pac-btn-ghost">취소</a>
        <button type="submit" class="pac-btn">등록하기</button>
      </div>
